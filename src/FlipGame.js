...
import GameCard from "./AnimGameCard";
...
function App(props, _children) {
  ...
  React.useEffect(() => {
    ...
    flip(faceUp_init);
    var cardLocked_init = Array.from(
      "x".repeat(opts.n_pairs * 2),
      (_g) => false
    );
  ...
  var isFaceUp = (i) => faceUp && i < faceUp.length && faceUp[i];
  ...
  function picked(y) {
    if (prevFlip === y || boardLocked || cardLocked[y]) {
      return;
    }
    setFaceUp(y, true);
  ...
  return (
    <div className="container-fluid">
        ...
        <div className="col-lg-9 col-sm-12">
          <div className="d-flex flex-row">
            {scores.map((val, i) => (
              <Card
                key={i}
                className={turn === i ? "turn-indicator" : "not-turn"}
              >
                <Card.Header>
                  <h3>{opts.playerName[i]}</h3>
                </Card.Header>
                <Card.Body>
                  <Card.Text>
                    <h2>{val}</h2>
      ...
      <div className="row d-flex flex-row">
        {imgs.map((val, i) => (
          <GameCard
            ...
            k={i}
            flip_card={picked}
            src={isFaceUp(i) ? val : faceDownImg}
          />
        ))}
      </div>
      <div className="row d-flex">
        <RightToast
          show={showRight}
          onClose={(e) => setShowRight(false)}
          bonus={bonus - 1}
        />
        <WrongToast show={showWrong} onClose={(e) => setShowWrong(false)} />
      </div>
      <GameOverModal
        show={matchPairCount >= opts.n_pairs}
        onHide={(e) => setPlayin(false)}
        winner_text={getWinner()}
      />
    </div>
  );
}
